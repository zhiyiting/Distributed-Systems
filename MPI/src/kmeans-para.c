...
int main(int argc, char **argv) {
	...
	if (rank == 0) {
    	...
    	for (int i = 1; i < num_tasks; i++) {
    		...
    		if (remaining_point < points_per_machine) {
    			send_count = remaining_point;
    		}
    		rc = MPI_Send(&send_count, 1, MPI_INT, i, 1, MPI_COMM_WORLD);
    		rc = MPI_Send(dataset, send_count, point_type, i, 2, MPI_COMM_WORLD);
    	...
    	do {
    		...
    		for (int i = 1; i < num_tasks; i++) {
    			rc = MPI_Receive(temp, k, cluster_type, i, 5, MPI_COMM_WORLD);
    			for (int j = 0; j < k; j++) {
		...
		while (startflag) {
			...
			cluster_sum_t *cluster_sum = compute_cluster_sum(node_t **cluster, int k) 
			rc = MPI_Send(cluster_sum, k, cluster_type, rank, 5, MPI_COMM_WORLD, &Stat);
			free(cluster_sum);
			free(cluster);
			rc = MPI_Recv(&startflag, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &Stat);
		}
